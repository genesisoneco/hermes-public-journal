...
# RIFE frame interpolation (portable, Vulkan GPU; ships rife-v4.6), used by 035_interpolate:
curl -L -o bin/rife.zip https://github.com/nihui/rife-ncnn-vulkan/releases/download/20221029/rife-ncnn-vulkan-20221029-windows.zip
unzip bin/rife.zip -d bin && rm bin/rife.zip
```
...
.venv/Scripts/python build.py --from 035      # redo the RIFE in-betweens, then pack and preview (about 4 min)
.venv/Scripts/python build.py --no-smooth     # no in-betweens and no trinity-smooth-* pages
.venv/Scripts/python 035_interpolate.py --anim walk_R,hack   # redo some anims, keep the rest
```
...
| `035_interpolate.py` | Runs after 04. It makes AI in-betweens with RIFE v4.6 so Trinity anims play at about 24 fps (CONTRACT §4 `"s"`). It covers locomotion, special, emote and utility anims with 3 or more distinct frames, except sleep. It adds k in-betweens per played pair (plus last→first for loops), with `fps×(k+1)≈24` and at most 24 frames, so the cycle length is unchanged. Frames are composited by anchor on one canvas. RIFE runs twice, once over #00FF00 and once over #0000FF, and triangulation matting of the two runs gives the alpha. That alpha is min'd with a G-dominance key and with an SDF-morphed, dilated mask of the two source alphas. Edge colour comes from the nearest clean interior pixel. The gate holds the nearest source frame in these cases: whole pairs that face opposite ways or to/from a back view (`facing`); pairs where more than 10% of the silhouette can't be explained by a local shift, meaning props appear or vanish or a face swaps, which RIFE could only cross-dissolve (`dissolve`); frames with IoU < 0.85 against the t-blended source alphas (`iou`); frames with more than 0.5% key or magenta edge fringe over the sources' own (`fringe`); and frames with more blobs than either source (`ghost`). In-betweens are shared by name (`<row>_s<a>_<b>_<j>_<k+1>`, reused backwards for b→a). Outputs go to `out/smooth/` and `out/smooth_qa.json`. |
| `05_pack.py` | MaxRects packing (BSSF) with 2px padding and 1px extrusion. Pages: trinity-core, trinity-ext, fx, ui. Writes the WebP pages (push-pull fill under transparency) and quantized PNG fallbacks, plus `atlas.json` with 1/4-res hit masks. WebP quality steps down until the budget holds: total under 1.5 MB, core@2x under 700 KB. The smooth frames then go on their own pages, `trinity-smooth-core` and `trinity-smooth-ext`, at the same quality, with a budget of @1x ≤ 1.6 MB and @2x ≤ 3.5 MB. The existing pages are not touched. |
| `06_preview.py` | Builds `out/preview.html` (player, flags, mirror test, source/smooth side-by-side view with a per-anim toggle), `out/compare_<anim>.{webp,gif,png}` for walk_F, walk_R, idle_F, hack and dance, `out/contact.png` and `out/qa.json` from the final WebP pages. Fails if any Trinity frame has more than 0.5% magenta edge px. |
